...
class MetaSTNN(nn.Module):
    ...
    def build(self):
        ...
        # self.predictor = nn.Linear(self.task_args['his_num'], self.task_args['pred_num'])
        self.MLP_layer1 = nn.Linear(self.task_args['his_num'], neural_num)
        self.MLP_layer2 = nn.Linear(neural_num, neural_num)
        self.predictor = nn.Linear(neural_num, self.task_args['pred_num'])
...
#         meta_graph_structure = softmax_func(torch.relu(meta_graph_structure))
        ...
        input = F.relu(input)
        MLP_1 = self.MLP_layer1(input)
        MLP_1 = F.relu(MLP_1)
        MLP_2 = self.MLP_layer2(MLP_1)
        MLP_2 = F.relu(MLP_2)
        output = self.predictor(MLP_2)

        # output = self.predictor(MLP_1)
